...
class LLM:
    def __init__(self, model_pth, torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True, load_dir=True):
        ...
        if load_dir:
            direction_dir = 'refusal_direction'
            direction_pth = os.path.join(direction_dir, f'{self.name}.pt')
            direction = torch.load(direction_pth)
            self.direction = direction/torch.norm(direction)
            direction_meta = json.load(open(os.path.join(direction_dir,  f'{self.name}.json'), 'r'))
    ...
    def get_proj_batch(self, instructions, batch_size=8):
        import math
