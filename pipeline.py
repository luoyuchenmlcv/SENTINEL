...
def jailbreak_score_pipeline(words_list, words_score_list, thresh_num, method=None):
    extracted_context = []
    ...
    jailbreak_scores = batch_jailbreak_score_perplexity_formated(model, extracted_context, method)
    ...
    for attack_config in methods:
        ...
        for test_idx, (k, t, g) in enumerate(zip(keys, test_cases, generations)):
            ...
            try:
                ...
                #############################################################################################################################
                words_A, word_scores_A, words_B, word_scores_B, mu_A, mu_B  = matching_score_pipeline2(model, t, g, save_pth =  f'experiment_data/text_heatmap/{str(test_idx).zfill(3)}.png', alpha = args.alpha, cw_num = args.cw_num)                 
                single_jailbreak_scores, extracted_context = jailbreak_score_pipeline([words_A], [word_scores_A], args.thresh_num, attack_config['ATTACK'])
                ...
                print(f"{test_idx}:run time: {run_time:3f} s, score: {final_score:.3f}")       
